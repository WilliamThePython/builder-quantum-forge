...
export class PolygonPartsExporter {
  ...
  static async exportPartsAsZip(
    ...
    const instructions = this.generateAssemblyInstructions(polygonFaces.length, { ...options, partThickness, polygonType });
  ...
  private static createPolygonSTL(
    ...
    const vertices = faceInfo.originalVertices.map((v: THREE.Vector3) => v.clone().multiplyScalar(scale));
    ...
    for (let i = 0; i < frontVertices.length; i++) {
      ...
      stlContent += this.addQuadToSTL(sideQuad[0], sideQuad[1], sideQuad[2], sideQuad[3]);
    }
   ...
   */
  private static addPolygonToSTL(vertices: THREE.Vector3[], normal: THREE.Vector3): string {
    let content = '';
  ...
  private static createPolygonOBJ(
    ...
    objContent += this.addPolygonToOBJ(backVertices.length, frontVertices.length + 1, 2, true);
   ...
   */
  private static calculatePolygonPartInfo(faceInfo: any, thickness: number, scale: number) {
    const vertices = faceInfo.originalVertices.map((v: THREE.Vector3) => v.clone().multiplyScalar(scale));
  ...
  private static generateStatistics(partData: any[]) {
    const faceTypeCounts = partData.reduce((acc, part) => {
  ...
  private static generateAssemblyInstructions(partCount: number, options: any): string {
    const date = new Date().toLocaleDateString();
...
`;
  ...
  static getExportStats(geometry: THREE.BufferGeometry, partThickness: number = 2): {
    ...
    const totalPrintMinutes = partCount * printTimePerPart * (partThickness / 2);
