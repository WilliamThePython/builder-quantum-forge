...
export class LargeFileOptimizer {
  ...
  private static async processWithScheduler(
    ...
    const geometry = await this.parseWithYields(arrayBuffer, file.name, progressCallback);
  ...
  private static async processWithChunking(
    ...
    const geometry = await this.parseWithManualYields(arrayBuffer, file.name, progressCallback);
  ...
  private static async fastOBJParse(
    ...
    for (const line of lines) {
      ...
      if (lineCount % 10000 === 0) {
        ...
        progressCallback?.(progress, `Processing line ${lineCount}/${totalLines}`);
   ...
   */
  private static async optimizeGeometry(geometry: THREE.BufferGeometry): Promise<THREE.BufferGeometry> {
    // Basic optimizations
    this.basicOptimization(geometry);
  ...
  private static mergeVertices(geometry: THREE.BufferGeometry): THREE.BufferGeometry {
    ...
    console.log(`⚡ Vertex optimization: ${positions.length / 3} → ${newPositions.length / 3} vertices`);
