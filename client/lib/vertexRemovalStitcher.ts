...
export class VertexRemovalStitcher {
  ...
  /**
   * Validate and fix coplanarity of polygon faces after decimation
   */
  private static validatePostDecimationCoplanarity(polygonFaces: any[]): any[] {
    console.log('   🔍 POST-DECIMATION: Validating coplanarity...');

    const validatedFaces: any[] = [];
    const tolerance = 0.001;
    let nonCoplanarCount = 0;
    let fixedCount = 0;
    ...
    for (const face of polygonFaces) {
      ...
      if (this.isStrictlyCoplanar(vertices, tolerance)) {
        // Face is coplanar, keep it
        validatedFaces.push({
          ...face,
        ...
        nonCoplanarCount++;
        console.warn(`     Found non-coplanar ${face.type} face after decimation`);

        // Try to fix by splitting into triangles
        if (vertices.length > 3) {
          const triangles = this.splitNonCoplanarPolygon(vertices, face);
          ...
          // For triangles, just keep them (triangles are always coplanar)
          validatedFaces.push({
            ...face,
            originalVertices: vertices
          });
    ...
    if (nonCoplanarCount > 0) {
      console.log(`   📊 POST-DECIMATION: ${nonCoplanarCount} non-coplanar faces found, ${fixedCount} triangles recovered`);
    } else {
      console.log(`   ✅ POST-DECIMATION: All faces remain coplanar`);
    }
  ...
  /**
   * Strict coplanarity check for post-decimation validation
   */
  private static isStrictlyCoplanar(vertices: THREE.Vector3[], tolerance: number): boolean {
    ...
    if (normal.length() < tolerance) return false;

    normal.normalize();

    // Check all remaining vertices against this plane
    for (let i = 3; i < vertices.length; i++) {
      const toVertex = new THREE.Vector3().subVectors(vertices[i], v1);
      const distance = Math.abs(toVertex.dot(normal));
  ...
  /**
   * Split non-coplanar polygon into triangles
   */
  private static splitNonCoplanarPolygon(vertices: THREE.Vector3[], originalFace: any): any[] {
    const triangles: any[] = [];
    ...
    for (let i = 1; i < vertices.length - 1; i++) {
      ...
      triangles.push({
        ...
        normal: normal,
        triangleIndices: originalFace.triangleIndices || []
      });
    }
