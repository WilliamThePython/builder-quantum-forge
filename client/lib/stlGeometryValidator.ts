...
export class STLGeometryValidator {
  ...
  static validateGeometry(geometry: THREE.BufferGeometry): ValidationReport {
    ...
    stats.avgArea =
      stats.totalTriangles > 0 ? stats.avgArea / stats.totalTriangles : 0;
  ...
  private static validateTriangles(
    ...
    for (let i = 0; i < triangleCount; i++) {
      ...
      if (
        v1.distanceTo(v2) < 1e-6 ||
        v2.distanceTo(v3) < 1e-6 ||
        v3.distanceTo(v1) < 1e-6
      ) {
  ...
  private static validateVertices(
    ...
    if (stats.duplicateVertices > 0) {
      warnings.push({
        ...
        details:
          "This may affect geometry optimization but is usually not critical",
      });
    }
  }
