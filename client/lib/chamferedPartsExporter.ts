  ...
  static async exportChamferedPartsAsZip(
    ...
    for (let i = 0; i < polygonFaces.length; i++) {
      ...
      partDatabase.push({
        ...
        "Normal Vector X": chamferedFace.faceInfo.normal.x.toFixed(6),
        "Normal Vector Y": chamferedFace.faceInfo.normal.y.toFixed(6),
        "Normal Vector Z": chamferedFace.faceInfo.normal.z.toFixed(6),
        "Min Edge Angle (°)": Math.min(...chamferedFace.edges.map(e => e.edgeAngle)).toFixed(1),
        "Max Edge Angle (°)": Math.max(...chamferedFace.edges.map(e => e.edgeAngle)).toFixed(1),
        "Avg Chamfer Angle (°)": (chamferedFace.edges.reduce((sum, e) => sum + e.chamferAngle, 0) / chamferedFace.edges.length).toFixed(1),
