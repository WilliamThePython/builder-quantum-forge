  ...
  static checkBrowserCapabilities(): {
    ...
    if ('memory' in performance && (performance as any).memory) {
      ...
      if (availableMemory < 100 * 1024 * 1024) { // Less than 100MB available
        ...
        capabilities.suggestions.push('Low memory detected - consider using smaller files or closing other tabs');
      }
    }
    
    // Check for mobile devices
    if (this.isMobileDevice()) {
      capabilities.maxRecommendedSize = Math.min(capabilities.maxRecommendedSize, 20 * 1024 * 1024);
      capabilities.suggestions.push('Mobile device detected - recommend files under 20MB for best performance');
    }
    
    // Check for older browsers
    if (!this.hasModernFeatures()) {
      capabilities.maxRecommendedSize = Math.min(capabilities.maxRecommendedSize, 25 * 1024 * 1024);
      capabilities.suggestions.push('Browser compatibility - recommend files under 25MB');
    }
