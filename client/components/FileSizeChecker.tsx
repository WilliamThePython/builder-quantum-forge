...
export function FileSizeChecker({ file, onProceed, onCancel }: FileSizeCheckerProps) {
  if (!file) return null;
  ...
  const getIcon = () => {
    switch (category) {
      case 'small': return <CheckCircle className="h-4 w-4 text-green-500" />;
      case 'medium': return <HelpCircle className="h-4 w-4 text-blue-500" />;
      case 'large': return <AlertTriangle className="h-4 w-4 text-orange-500" />;
      case 'very-large': return <AlertTriangle className="h-4 w-4 text-red-500" />;
    }
  };
  ...
  return (
    <Alert variant={getAlertVariant()} className="mb-4">
      ...
      <AlertDescription className="space-y-3">
        <div>
          <p className="font-medium">Estimated processing time: {estimate.estimatedSeconds} seconds</p>
